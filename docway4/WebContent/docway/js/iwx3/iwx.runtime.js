...
IwxRuntime.Client = (function() {
	...
	// download potrebbe succedere in bulk, allora serve fare conteggio
	// questo contattore va settato prima del commando download, poi dopo in handler del download va
	// decrementato fino allo 0 e poi va fatta action
	var currentDownloadBulkSize = 0;
	// se ce bulk questo nuero cresce ogni volta che un file e stato caricato dal server
	// quando e pari al currentDownloadBulkSize vera effetuata vera azione nextActionAfterDownlaod
	var currentBulkProgressCount = 0;
	// quanti errori?
	var currentBulkProgressErrorsCount = 0;
	// se ce bulk qui ce true
	var workingInBulk = false;
	// bulk file list
	var currentBulkFiles = [];
	
	
	
	/**
	 * inizia un nuovo bulk
	 */
	function initBulk(files, nextAction) {
		currentBulkFiles = files || [];
		currentDownloadBulkSize = currentBulkFiles.length;
		nextActionAfterDownlaod  = nextAction;
		workingInBulk = true;
		currentBulkProgressCount = 0;
	}
	
	/**
	 * reseta variabili del bulk
	 */
	function resetBulk() {
		currentDownloadBulkSize = 0;
	...
	function getBulkFileTitleByGuid(guid){
		// add id to array iniziale in caso di bulkprint
		if(currentBulkFiles != null && currentBulkFiles != undefined){
			var ix = currentBulkFiles.findIndex(function(el){
				return el.guid == guid;
			})
			if(ix > -1){
				return currentBulkFiles[ix].title;
			} else {
				return 'unknown';
			}
		}
	...
	function getGoodBulkImages(){
		return currentBulkProgressCount - currentBulkProgressErrorsCount;
	}
	
	/**
	 * avvanza counter del bulk, e se NON FINITO ritorna true
	 * msg: {
	 * 			GUIDS: array<string>
	 * 			TEXT: "Error code:NNNNN"
	 * 		}
	 * isFail: true if error
	 */
	function advanceAndCheckBulk(msg, isFail) {
		if(currentBulkProgressCount < currentDownloadBulkSize) {
			++currentBulkProgressCount;
			
			// fai il messaggio
			if(isFail) {
				++currentBulkProgressErrorsCount;
				showMessage(currentBulkProgressCount + "/" + currentDownloadBulkSize + ": " + getBulkFileTitleByGuid(msg['GUIDS'][0]) + ' falito [' + msg['TEXT'] + ']', 'error', 'bulk');
			} else {
				showMessage(currentBulkProgressCount + "/" + currentDownloadBulkSize + ": " + getBulkFileTitleByGuid(msg['GUIDS'][0]) + " OK", 'info', 'bulk')
			}
			
			var done = currentBulkProgressCount == currentDownloadBulkSize; // se finito ritorna true
			if( done ) {
				// finisci lavoro
				finalizaBulk();
			...
			throw new Error("Errore nella operazione bulk! (iwx)");
		}
	...
	// finalizza bulk
	function finalizaBulk() {
		if (getGoodBulkImages() > 0) {
			if (nextActionAfterDownlaod == 'finalizeBulkPrint') {
				// prendiamo tutti i file aggiunti dal job
				iwx.getFilesList().done(function(x){
					x = x || [];								
					showMessage("File scaricati, inizio stampa ...", "info", "bulk");
					// alert('files pronti per la stampa: (' + x.join(',') + ')');
					iwx.printFile(x, 'default', {});
					hideOpenwaitmsg();
					showMessage("Selezionare la stampante dal dialogo di sistema e una volta completata l'attività chiudere la finestra.", "info", "bulk");
				}).fail(function(d){
					showMessage('Stampa massiva falita, chiudere la finestra.', 'error', 'bulk');
				});							
				nextActionAfterDownlaod = '';
			}
		} else {
			showMessage('Stampa massiva falita, nessun imagine stampabile, chiudere la finestra.', 'error', 'bulk');
		}	
	}
	...
	function isActiveIWX() {
		var active = false;
		if (iwx != undefined && iwx != null) {
			if (iwxVersion != null && iwxVersion != '')
				active = true;
		}
		return active;
	}
	...
	function getIwxDownloadMessage(msg, enablefilteralert) {
		...
		downloadURL += "/common/iwx/iwx3/setup_iwx.msi";
		...
		if (enablefilteralert) {
			...
			if (typeof window.external.msActiveXFilteringEnabled != "undefined")
				msghtml = msghtml
						+ '<br/><strong>Nel caso IWX risulti installato</strong> verificare il servizio IWX e la configurazione di <strong>ActiveX Filtering</strong> del browser. Per maggiori informazione consultare la <a href="'
						+ wikiURL + '" target="blank_">pagina WIKI</a>';
			else 
				msghtml = msghtml 
						+ '<br/><strong>Nel caso IWX risulti installato</strong> verificare che sia attivo il servizio IWX e di non avere attivi dei filtri sui plugin/activeX. Per maggiori informazione consultare la <a href="' 
						+ wikiURL + '" target="blank_">pagina WIKI</a>';
		}
	...
	function showErrorMessage(texterror, level) {
		if (!level)
			level = 'error';
		showMessage(texterror, level, 'error')
	...
	/**
	 * aggiunge messaggio nella area di messaggi
	 */
	function showMessage(text, level, destKind) {
		var destId = '#iwx_' + destKind + '_messages'
		var msgs = $(destId).html();
		if (!level)
			level = 'info';
		if (text != undefined && text != null && text != '') {
			text = '<div class="' + level + '">' + text + '</div>';
			$(destId).html(msgs + text);
			if(destKind == 'bulk') {
				setTimeout(function() {
					var el = $(destId)[0];
	...
	function showImageDescriptor() {
		iwx.image.getImageDescriptor().done(function(x){
			var descrImage = x || [];
			...
			// Se decommentato provoca problemi nell'esecuzione del plugin IWX
	 ...
	 * su eXtraWay
	 */
	function uploadFilesIWX(isMergeAction) {
		if (isMergeAction)
	...
	function uploadDoneIWX(uploadResponse) {
		iwx.getFileDescriptor(uploadResponse.GUIDS[0]).done(function(x){
			var curFileDescriptor = x || [];
			...
			// TODO traduzioni
			alert("Errore di comunicazione con il componente IWX.");
		})
	}
	...
	function openImageIWX(indice) {
		if (isActiveIWX()) {
			...
			var guid = $('#templateForm\\:xwImages_' + indice + '_iwxGuId')
					.val();
			if(guid.match(/^iwx[{(]?[0-9A-F]{8}[-]?(?:[0-9A-F]{4}[-]?){3}[0-9A-F]{12}[)}]?$/i)) {
				iwx.showFile(guid);
			} else {
				alert('File aggiunto via Drag And Drop non e disponibile per visualizzazione.');
			}	
		}
	...
	function acquireDocsFromScanner() {
		...
		if (guidsToUpload != null && guidsToUpload.length > 0) {
			...
			if (guid != null && guid != '') {
				...
				if (curGuid != undefined && curGuid != null) {
					iwx.getFileDescriptor(curGuid).done(function(x) {
	...
	/**
	 * Caricamento tramite viewer.js della preview dell'immagine con adattamento alla larghezza dell'area di visualizzazione
	 */
	function previewImagePage() {
		setTimeout(showImageDescriptor(), 1);
		//setTimeout(adaptToWidthIWX(), 3000); // adattamento alla larghezza dopo X secondi (3000 = 3 sec.)
	}
	...
	function configPrinterIWX(profilostampa) {
		if (isActiveIWX()) {
			...
			iwx.printFile(null, profilostampa, text);
	...
	ret = {
		...
		iwxCreatedHandler : function() {
			...
			// @since iwx 3.0.11.0 resetJob return promise, event is removed!!!
			...
			*/
				...
				if (nextActionAfterUpload == 'acquireDocsFromScanner') {
					...
					if (errorCode == "1016") { // dimensione del file eccessiva
						...
						if (resp['GUIDS'] && resp['GUIDS'][0]) {
							iwx.getFileDescriptor(resp.GUIDS[0]).done(function(x) {
								var curFileDescriptor = x || [];
			...
			iwx.addEvent('AddFilesSuccessfully', function(x) {
				// x puo contenere un singolo id oppure array dei id
				var what = x["GUIDS"].constructor === Array ? x["GUIDS"] : [x["GUIDS"]];
				var countDown = what.length;
				for(var i in what) {
					// prendiamo descrittori 1 ad 1 e aggiungiamo delle url costruendogli con variabili che abbiamo a disposizione
					
					iwx.getFileDescriptor(what[i]).done(function(x){
						// iwx file descriptor
						/* esempio troncato sulle variabili rilevanti
						 * {  
							   "GUID":"iwx0b496e7e-8097-4400-8c46-e348bc7e52b2",
							   "HTTP.Host":null,
							   "HTTP.Login":null,
							   "HTTP.MaxFileSize":null,
							   "HTTP.Proxy":null,
							   "HTTP.Pwd":null,
							   "NAME":"iwx0b496e7e-8097-4400-8c46-e348bc7e52b2.xml",
							   "PATH":"C:/Users/Laco/AppData/Local/Temp/iwx/CACHE/HTTP/iwx0b496e7e-8097-4400-8c46-e348bc7e52b2.xml",
							   "QRCODE":null,
							   "STATUS":50,
							   "TYPE":0,
							   "XWID":"jskT7ZvFazw=.xml"
							}
						 */
						var cd = x || {};
						
						// cambio host a download
						var downloadHostHTTP = hostHTTP.replace("/fileupload", "/filedownload");
						// faccio la url
						var downloadHost = downloadHostHTTP
									+ "?name="
									+ encodeURIComponent(cd["XWID"]) 
									+ "&title=" 
									+ encodeURIComponent(cd["XWID"].replace(/\//g, ''))
									+ "&login=" + userLogin + "&matricola="
									+ userMatricola + "&_cd=" + escape(customTupleName) + "&db=" + dbName
									+ "&physDoc=" + encodeURIComponent($('#physDocHidden').text()); // TODO senza parametri in get non funziona (bug??)
						
						// setto guid in form cosi rimane, se pero si fa con array alla fine ultimo vera preso
						$('#templateForm\\:xwImages_' + currentDownloadIndex + '_iwxGuId').val(cd["GUID"]);
						//currentDownloadIndex = ''; // TODO non dovrebbe essere necessario azzerare il valore... il file selezionato rimane sempre lo stesso.
						
						// add id to array iniziale in caso di bulkprint
						if(currentBulkFiles != null && currentBulkFiles != undefined){
							var ix = currentBulkFiles.findIndex(function(el){
								return el.name == cd["XWID"];
							})
							if(ix > -1){
								currentBulkFiles[ix].guid = cd["GUID"];
							}
						}
			
						iwx.setParam(cd["GUID"], 'HTTP.Host', downloadHost);
						// decrementa counter, se arriva a 0 abbiamo fatto anche ultimo
						--countDown;
						if(countDown <= 0) {
							if(workingInBulk) {
								showMessage("Download file in corso ...", 'info', 'bulk');
							}
							// fai download finale di tutti
							iwx.http.downloadFiles(what, { // TODO non funziona il passaggio di parametri in downlod!!!
								'matricola' : userMatricola,
								'login' : userLogin,
								'_cd' : customTupleName,
								'db' : dbName
			...
			iwx.addEvent('HTTPDownloadSuccessfully', function(x) {
				// manage eventual bulk
				if (workingInBulk) {
					if(!advanceAndCheckBulk(x, false)) {						
						// finito
						hideOpenwaitmsg();
					}					
				}else{
			...
			iwx.addEvent('HTTPDownloadFailed', function(x) {
				// se ce un bulk in corso va avvanzato anche quello!!!
				if (workingInBulk) {
					// mo lo facciamo solo avanzare, perche non ci interessa nulla altro
					if(!advanceAndCheckBulk(x, true)){
						// finito
						hideOpenwaitmsg();
					}
				} else {
			...
			// eventi relativi alla visualizzazione di immagini
			iwx.addEvent('ShowFileSuccessfully', function(x) {
				// TODO questa attivita' andrebbe eseguita solo in caso di apertura di immagini (attualmente l'unica possibile)
				previewImagePage();
			});
			iwx.addEvent('FirstPageSuccessfully', function(x) {
				previewImagePage();
			});
			iwx.addEvent('PrevPageSuccessfully', function(x) {
				previewImagePage();
			});
			iwx.addEvent('NextPageSuccessfully', function(x) {
				previewImagePage();
			});
			iwx.addEvent('LastPageSuccessfully', function(x) {
				previewImagePage();
			});
		...
		iwxCreationFailedHandler : function(error) {
			...
			if (actionToPerform != '') {
				if (actionToPerform == 'downloadCurrentImage') {
					...
					if (xwimages_length > 1) {
						...
						// qui facciamo il controllo formale se guid e guid del iwx
						if (lastGuid != '' && lastGuid.match(/^iwx[{(]?[0-9A-F]{8}[-]?(?:[0-9A-F]{4}[-]?){3}[0-9A-F]{12}[)}]?$/i)) {
							IwxRuntime.Client.openImageIWX(xwimages_length-2);
						} 
					}
				}
				else if (actionToPerform == 'doBulkPrint') {
					
					displayOpenwaitmsg(); // visaulizzazione dell'icona di attesa (inizio attivita' di stampa)
					
					showMessage("Aggiunta di " + currentBulkFiles.length + " immagini alla coda di stampa ...", 'info', 'bulk');
					
					if (currentBulkFiles != null && currentBulkFiles != undefined && currentBulkFiles.length > 0) {
						iwx.http.addFiles({
							FileList : currentBulkFiles.map(function(el) {
								return el.name;
							})
						});
					} else {
			...
			try {
				...
				IWxApi.initialize(
					...
					"iwx_holder", 
					{
						...
						height : heightIWX, 
						debugLog : false, 
						handleImagesBinary : true
					}
				)
				.done(function(_iwx){
					iwx = _iwx;
					iwx.getVersion().done(function(d){ 
						iwxVersion = d.version; 
						IwxRuntime.Client.iwxCreatedHandler();
					});
				}).fail(function(){
					// errore in initialize
					
					//iwx.getVersion().done(function(d){ 
					//	IwxRuntime.Client.iwxCreationFailedHandler();
					//});
					showErrorMessage(getIwxDownloadMessage('Occorre scaricare e installare il plugin IWX', true));
				});
				
				/*
				$j('#iwx_holder').each(function(){
				    var $div = $j(this);
