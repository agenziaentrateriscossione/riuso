...
.clear { 
        clear: both; 
			...
			<body>
			<!-- dati protocollo: inizio -->
			<div id="protocolloheader">
				<xsl:call-template name="ProtocolloInformatico"/>
			</div>
			<!-- dati protocollo: fine -->
	...
	<xsl:template name="Line">
		...
		<div id="orderline" class="orderline">
			<table width="100%" cellspacing="0" cellpadding="3" class="ntable" summary="Order lines">
				<tr height="25">
					...
					<xsl:if test="cac:LineItem/cac:Item/cac:AdditionalItemProperty">
						<tr>
							...
							<td colspan="8" class="hdrcol23">
								...
								<table summary="Additional property" border="0" width="100%" cellpadding="0" cellspacing="0">
...
<xsl:template name="ResponseLine">
		...
		<div id="responseline" class="orderline">
			<table width="100%" cellspacing="0" cellpadding="3" class="ntable" summary="Response lines">
				<tr height="25">
			...
			<table width="100%" cellspacing="0" cellpadding="0" summary="Note risposta">
			<tr>
...
<xsl:template name="Totals">
            <table width="100%" cellspacing="0" cellpadding="0" summary="Totale ordine">
			<tr>
				  <td width="4%" class="hdrcol23" align="right" nowrap="nowrap">&#160;</td>
...
<xsl:template name="Attachment">
	...
	<xsl:template name="ProtocolloInformatico">
		<xsl:if test="DatiSegnatura">
			<div class="font7 protinf">
				<xsl:text>PROTOCOLLO.</xsl:text>
				<xsl:choose>
					<xsl:when test="DatiSegnatura/TipoDocumento='arrivo'">E</xsl:when>
					<xsl:when test="DatiSegnatura/TipoDocumento='partenza'">U</xsl:when>
					<xsl:otherwise>I</xsl:otherwise>
				</xsl:choose>
				<xsl:text>.</xsl:text>
				<xsl:call-template name="FormatNumProt">
					<xsl:with-param name="ExtendeNumProt" select="DatiSegnatura/NumeroProtocollo" />
				</xsl:call-template>
				<xsl:text>.</xsl:text>
				<xsl:if test="DatiSegnatura/DataProtocollo">
					<xsl:choose>
						<xsl:when test="string-length(DatiSegnatura/DataProtocollo) = 8">
							<xsl:call-template name="FormatDateYYYYMMDD">
								<xsl:with-param name="DateTime" select="DatiSegnatura/DataProtocollo" />
							</xsl:call-template>
						</xsl:when>
						<xsl:otherwise>
							<xsl:value-of select="DatiSegnatura/DataProtocollo" />
						</xsl:otherwise>
					</xsl:choose>
				</xsl:if>
			</div>
	...
	<xsl:template name="FormatNumProt">
		...
		<xsl:value-of select="substring($ExtendeNumProt,14)" />
