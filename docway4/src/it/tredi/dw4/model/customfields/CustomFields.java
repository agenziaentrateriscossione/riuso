...
public class CustomFields extends XmlEntity {
	...
	public String evaluateRelationships() {
		...
		if (this.relationships != null && !this.relationships.isEmpty()) {
			...
			// mbernardini 23/10/2019 : l'azzeramento del valore dei campi custom non visualizzati deve essere fatta al termine 
			// della fase di valutazione delle regole derivanti da relazioni fra campi
			for (Section section : this.sections) {
				if (section != null) {
					for (Field field : section.getFields()) {
						if (field != null && !field.isVisible()) {
							for (FieldInstance instance : field.getInstances()) {
								instance.setValue("");
							}
						}
					}
