...
import it.tredi.dw4.docway.model.procedimenti.Riconciliazione;
...
public class Extra extends XmlEntity {
	...
	// mbernardini 09/03/2020 : gestione della legislatura
	private Legislatura legislatura = new Legislatura();
	
	// mbernardini 09/03/2020 : gestione dei procedimenti API
	private List<Procedimento> procedimenti;
	
	// mbernardini 27/03/2020 : gestione delle riconciliazioni da API
	private Riconciliazione riconciliazione = new Riconciliazione();
	
	// mbernardini 16/03/2020 : gestione di eventuali metadati scritti da API tramite webservices e mostrati in stile note
	private String meta_api_text;
	...
	public XmlEntity init(Document dom) {
		...
		// gestione della legislatura
		legislatura.init(XMLUtil.createDocument(dom, "extra/legislatura")); // gestione dei campi extra
		// gestione dei procedimenti
		this.procedimenti = XMLUtil.parseSetOfElement(dom, "extra/procedimenti/procedimento", new Procedimento());
		// gestione di metadati registrati da API tramite WS
		meta_api_text = assignExtraParam(dom, "extra/meta_api_text", false, "meta_api_text", false);
		// gestione delle riconciliazioni
		this.riconciliazione.init(XMLUtil.createDocument(dom, "extra/riconciliazione"));
...
//			params.put(prefix+".NA_codiceNorma.@NA_dataEmanazioneNorma", na_dataEmanazioneNorma);
		...
		// gestione della legislatura
		if (legislatura.getNumero() != null && !legislatura.getNumero().isEmpty())
			params.putAll(legislatura.asFormAdapterParams(prefix+".legislatura"));
		// gestione dei procedimenti/riconciliazioni
		// N.B.: I PROCEDIMENTI SONO GESTITI SOLO IN FASE DI LETTURA. IN SCRITTURA NON DEVE MAI ESSERE INVIATO NULLA AL SERVICE
...
//	}
	...
	public List<Disposizione> getNa_disposizioni() {
		return na_disposizioni;
	}

	public void setNa_disposizioni(List<Disposizione> na_disposizioni) {
		this.na_disposizioni = na_disposizioni;
	}

	public List<UnitaRichiamata> getNa_unita() {
		return na_unita;
	}

	public void setNa_unita(List<UnitaRichiamata> na_unita) {
		this.na_unita = na_unita;
	}
	...
	public String getMeta_api_text() {
		return meta_api_text;
	}

	public void setMeta_api_text(String meta_api_text) {
		this.meta_api_text = meta_api_text;
	}
