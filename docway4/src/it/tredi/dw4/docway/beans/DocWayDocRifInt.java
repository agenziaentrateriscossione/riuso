...
public class DocWayDocRifInt extends DocWayDocedit {
	...
	//dpranteda 30/11/2019 : flag per indicare che l'assegnazione di OP è fatta per il flusso PEC MANAGER
	private boolean forPecManager = false;
	
	public boolean isForPecManager() {
		return forPecManager;
	}

	public void setForPecManager(boolean forPecManager) {
		this.forPecManager = forPecManager;
	}
	...
	public String confirmAddOP() throws Exception{
		if(forPecManager) {
			// dpranteda 30/11/2019 : assegnazione OP da flusso PEC MANAGER che cambia anche lo stato al documento
			this.formsAdapter.confirmRifIntForPecManager("OP", doc.isSendMailRifInterni(), doc.isSendMailSelectedRifInterni(), doc.getNotificheCapillariParam(), false, doc.getAssegnazioneOPParam());
		}
		else
		{
