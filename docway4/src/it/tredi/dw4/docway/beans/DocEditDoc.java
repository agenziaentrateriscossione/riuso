...
public abstract class DocEditDoc extends DocWayDocedit {
	...
	// tiommi 16/10/2019 : modifica oggetto protocollo con diritto
	private boolean richiestaMotivazioneOggetto = false;	// flag apertura modale inserimento motivazione
	private String motivazioneCambioOggetto = ""; 			// valore motivazione
	...
	public boolean isRichiestaMotivazioneOggetto() {
		return richiestaMotivazioneOggetto;
	}

	public void setRichiestaMotivazioneOggetto(boolean richiestaMotivazioneOggetto) {
		this.richiestaMotivazioneOggetto = richiestaMotivazioneOggetto;
	}

	public String getMotivazioneCambioOggetto() {
		return motivazioneCambioOggetto;
	}

	public void setMotivazioneCambioOggetto(String motivazioneCambioOggetto) {
		this.motivazioneCambioOggetto = motivazioneCambioOggetto;
	}
...
//		}
		...
		// mbernardini 06/03/2020 : in caso di gestione procedimenti abilitata occorre verificare la valorizzazione del campo tipologia (obbligatorio)
		if (getFormsAdapter().checkBooleanFunzionalitaDisponibile("procedimentiApi", false) 
								&& !isFatturaPaAttiva()) { // solo se non si tratta di una fatturaPA
			if (getDoc().getTipologia() == null || getDoc().getTipologia().getCod() == null || getDoc().getTipologia().getCod().trim().isEmpty()) {
				this.setErrorMessage("templateForm:codTipologiaDoc", I18N.mrs("dw4.occorre_selezionare_una_tipologia_del_documento"));
				result = true;
			}
		}
	...
	/**
	 * Torna TRUE se il documento fa riferimento ad una fatturaPA attiva, FALSE altrimenti
	 * @param doc
	 * @return
	 */
	private boolean isFatturaPaAttiva() {
		return codiceRepertorio != null && codiceRepertorio.equalsIgnoreCase("FTRPAA"); 
	}
	...
	/**
	 * Metodo chiamato solo da docEditModify dei protocolli quando la funzionalità abilitaModificaOggettoProtocolloConDiritto
	 * è attiva, controlla se l'oggetto è stato modificato, in caso positivo apre la modale di inserimento della motivazione e
	 * rimanda il salvataggio a questa, altrimenti procede con il normale salvataggio.
	 * @return
	 * @throws Exception
	 */
	public String checkIfOggettoProtChanged() throws Exception {
		// pre check di validazione
		if (checkRequiredField()) return null;
		String oggetto = getDoc().getOggetto().replaceAll("\r\n", "\n");
		String oggettoOld = getDoc().getOggettoFromResponse().replaceAll("\r\n", "\n");
		if (!oggetto.equals(oggettoOld)) {
			// oggetto di un protocollo modificato, richiesta di motivazione
			richiestaMotivazioneOggetto = true;
			return null;
		} else {
			return saveDocument();
		}
	...
	/**
	 * L'utente chiude la modale di inserimento della motivazione di modifica dell'oggetto, occorre ripristinare
	 * il vecchio oggetto.
	 * @return
	 * @throws Exception
	 */
	public String ripristinaOggettoProtocollo() throws Exception {
		getDoc().setOggetto(getDoc().getOggettoFromResponse());
		motivazioneCambioOggetto = "";
		richiestaMotivazioneOggetto = false;
		return null;
	}

	/**
	 * L'utente richiede il salvataggio del documento, controllo su presenza di motivazione e richiesta di salvataggio
	 * in caso passi la validazione.
	 * @return
	 * @throws Exception
	 */
	public String salvaDocumentoConModificaOggetto() throws Exception {
		if (motivazioneCambioOggetto == null || motivazioneCambioOggetto.isEmpty()) {
			// errore validazione motivazione obbligatoria
			this.addErrorFieldId("templateForm:motivazione_oggetto_text");
			return null;
		} else {
			// procede con il salvataggio inserendo parametri aggiuntivi relativi al precedente oggetto
			this.formsAdapter.getDefaultForm().addParam("modificaOggettoProtocollo", "true");
			this.formsAdapter.getDefaultForm().addParam("motivazioneCambioOggettoProtocollo", motivazioneCambioOggetto);
			this.formsAdapter.getDefaultForm().addParam("oggettoPrecedente", getDoc().getOggettoFromResponse());
			motivazioneCambioOggetto = "";
			richiestaMotivazioneOggetto = false;
			String result = saveDocument();
			// errore di validazione o errore lato server, forzo la rimozione del parametro appena aggiunto
			if (result == null) {
				this.formsAdapter.getDefaultForm().removeParam("modificaOggettoProtocollo");
				this.formsAdapter.getDefaultForm().removeParam("motivazioneCambioOggettoProtocollo");
				this.formsAdapter.getDefaultForm().removeParam("oggettoPrecedente");
			}
			return result;
		}
	}
	...
	/**
	 * Calcola attravero il parsing della vecchia Response xml se i rif_esterni sono stati modificati
	 *
	 * @return TRUE se i rif_esterni sono stati modificati, FALSE altrimenti
	 * @throws DocumentException
	 */
	@SuppressWarnings("unchecked")
	public boolean isRifEsterniChanged() throws DocumentException {
		List<RifEsterno> oldRifEsterni = XMLUtil.parseSetOfElement(DocumentHelper.parseText(xml), "/response/doc/rif_esterni/rif", new RifEsterno());
		List<RifEsterno> newRifEsterni = (getDoc().getRif_esterni() != null) ? getDoc().getRif_esterni() : new ArrayList<>();
		if (oldRifEsterni.size() != newRifEsterni.size())
			return true;
		Map<String, String> oldFormsAdapters = new HashMap<>();
		Map<String, String> newFormsAdapters = new HashMap<>();
		String prefix = "rif_esterno_check[";
		for (int i = 0; i < oldRifEsterni.size(); i++) {
			RifEsterno rifEsterno = oldRifEsterni.get(i);
			oldFormsAdapters.putAll(rifEsterno.asFormAdapterParams(prefix + i + "]"));
		}
		for (int i = 0; i < newRifEsterni.size(); i++) {
			RifEsterno rifEsterno = newRifEsterni.get(i);
			newFormsAdapters.putAll(rifEsterno.asFormAdapterParams(prefix + i + "]"));
		}
		if (oldFormsAdapters.size() != newFormsAdapters.size())
			return true;
		for (Map.Entry<String, String> oldEntry : oldFormsAdapters.entrySet()) {
			String newValue = newFormsAdapters.get(oldEntry.getKey());
