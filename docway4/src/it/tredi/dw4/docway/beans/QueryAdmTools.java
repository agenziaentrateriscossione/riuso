...
public class QueryAdmTools extends DocWayQuery {
	...
	private String fileNameLegislature = "";
	...
	public String getFileNameLegislature() {
		return fileNameLegislature;
	}

	public void setFileNameLegislature(String fileNameLegislature) {
		this.fileNameLegislature = fileNameLegislature;
	}
	...
	/**
	 * Download del file di descrizione delle legislature
	 *
	 * @return
	 * @throws Exception
	 */
	public String downloadFileDescrizioneLegislature() throws Exception {
		try {
			formsAdapter.downloadSpecificFile("legislatureLocation");
			AttachFile attachFile = getFormsAdapter().getDefaultForm().executeDownloadFile(getUserBean());
	...
	/**
	 * Upload del file di descrizione delle legislature
	 *
	 * @return
	 * @throws Exception
	 */
	public String uploadFileDescrizioneLegislature() throws Exception {
		try {
			if (fileNameLegislature == null || fileNameLegislature.equals("")) {
				this.setErroreResponse(I18N.mrs("dw4.occorre_selezionare_il_file_da_importare"), Const.MSG_LEVEL_WARNING);
				return null;
			}
			...
			UserBean userBean = getUserBean();
			try {
				File fileLegislature = UploadFileUtil.getUserTempFile(fileNameLegislature, userBean.getLogin(), userBean.getMatricola());

				formsAdapter.uploadSpecificFile("legislatureLocation");

				XMLDocumento response = getFormsAdapter().getDefaultForm().executeUploadRequestToXMLDocumento(userBean, "uploadFile", fileLegislature);
				
				if (handleErrorResponse(response)) {
					formsAdapter.fillFormsFromResponse(formsAdapter.getLastResponse()); //restore delle form
					return null;
				}
				//kveizi 06/02/2018 :  Messaggio di conferma al termine dell'attività di upload del file legislature.xml
				else {
					this.showMessageWarning(I18N.mrs("dw4.upload_completato_successo"), Const.MSG_LEVEL_SUCCESS);
				}
			}
