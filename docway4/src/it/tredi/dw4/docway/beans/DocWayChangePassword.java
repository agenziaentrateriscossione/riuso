...
public class DocWayChangePassword extends DocWayQuery {
	...
	public void setForcedChange(boolean forcedChange) {
		...
		try {
			...
			if(response.getRootElement().attributeValue("dbTable").equals("@pwdchanged")) {
				...
				if (this.forcedChange) {
					removeSessionAttribute("forceChangePassword");
					DocwayInit docwayInit = (DocwayInit) getSessionAttribute("docwayInit");
					UserBean userBean = getUserBean();
					if (docwayInit != null && userBean != null) {
						return docwayInit.forceReInit(userBean);
					}
				}
				return null;
			}
