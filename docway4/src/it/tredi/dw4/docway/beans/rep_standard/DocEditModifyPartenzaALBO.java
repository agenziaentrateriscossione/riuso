 ...
 */
public class DocEditModifyPartenzaALBO extends DocEditModifyPartenza {

	// campi specifici del repertorio di richiesta pubblicazione albo online
	private Richiedente richiedente = new Richiedente();
	
	public DocEditModifyPartenzaALBO() throws Exception {
		super();
	}
	...
	public String saveDocument() throws Exception {
		try {
			...
			boolean isProtocollato = !getDoc().isBozza() && getDoc().getNum_prot() != null && !getDoc().getNum_prot().isEmpty();
			if (!abilitaModificaDatiDiProtocollo && !abilitaModificaMittenteConDiritto && isProtocollato)
				rifEsterniModificabili = false;
			// controllo se i rif_esterni sono stati modificati nel protocollo
			if (abilitaModificaMittenteConDiritto && isProtocollato && (isRifEsterniChanged() || isRichiedenteChanged())) {
				formsAdapter.addRifEsterniChangedInfos("modifica_richiedente");
			}
			...
			ShowdocPartenzaALBO showdocPartenzaALBO = new ShowdocPartenzaALBO();
			showdocPartenzaALBO.getFormsAdapter().fillFormsFromResponse(response);
			showdocPartenzaALBO.init(response.getDocument());
			showdocPartenzaALBO.loadAspects("@partenza", response, "showdoc");
			setSessionAttribute("showdocPartenzaALBO", showdocPartenzaALBO);
			
			return "rep_standard_showdoc@partenza@ALBO@reload";
		}
		catch (Throwable t) {
	...
	/**
	 * Controlla se il richiedente interno è cambiato
	 *
	 * @return TRUE se il richiedente interno è stato modificato, FALSE altrimenti
	 */
	private boolean isRichiedenteChanged() throws DocumentException {
		Document dom = DocumentHelper.parseText(xml);
		Richiedente oldRichiedente = new Richiedente();
		oldRichiedente.init(XMLUtil.createDocument(dom, "/response/doc/extra/richiedente"));
		Richiedente newRichiedente = richiedente;
		String prefix = "richiedente_check";
		Map<String, String> oldFormsAdapters = oldRichiedente.asFormAdapterParams(prefix);
		Map<String, String> newFormsAdapters = newRichiedente.asFormAdapterParams(prefix);
		...
		for (Map.Entry<String, String> oldEntry : oldFormsAdapters.entrySet()) {
			...
			if (newValue == null)
				return true;
			if (!newValue.equals(oldEntry.getValue()))
				return true;
		}
		return false;
	}
