...
public class QueryFascicolo extends DocWayQuery {
	...
	private boolean itftiporuolo = false;
	...
	private String range_fascnrecord_from;
	private String range_fascnrecord_to;
	...
	private List<FascicoloSpecialeInfo> fascicoloSpecialeInfoList = new ArrayList<FascicoloSpecialeInfo>();
	...
	private String valueFascicolazioneSpecialeMassiva = "";
	
	private int[] numfascLengths;
	...
	public void init(Document dom, String fascicoloSpecialeId) {
		this.xml = dom.asXML();
		...
		// mbernardini 11/05/2017 : ricerca tramite gerarchia fascicoli valida solo se non attivo elasticsearch
		sottofascicoli = formsAdapter.checkBooleanFunzionalitaDisponibile("mostraArticolazioneSottoFasc", false) && !formsAdapter.isElasticsearchEnabled();

		if(fascicoloSpecialeId.isEmpty())
			fascicoloSpecialeInfo.init(XMLUtil.createDocument(dom, "/response/fascicolo_speciale_info"));
		else
			fascicoloSpecialeInfo.init(XMLUtil.createDocument(dom, "/response/fascicolo_speciale_info[@id='"+fascicoloSpecialeId+"']"));

		fascicoloSpecialeInfoList = XMLUtil.parseSetOfElement(dom, "/response/fascicolo_speciale_info", new FascicoloSpecialeInfo());
		...
		valueFascicolazioneSpecialeMassiva = "customSearch";
		
		numfascLengths = NumFascUtils.getNumFascLengths(XMLUtil.parseStrictAttribute(dom, "/response/@num_fasc_length"));
    }
	...
	protected String createQuery(boolean ignoreClassifDescription) throws Exception {
		...
		if( (range_fascnrecord_from !=  null && range_fascnrecord_from.trim().length()>0) || (range_fascnrecord_to != null && range_fascnrecord_to.trim().length()>0)){
        	String complete_nrecord_from = "";
        	if (range_fascnrecord_from != null && range_fascnrecord_from.length() > 0)
        		complete_nrecord_from = StringUtil.fillString(range_fascnrecord_from, "0", 7);
        	String complete_nrecord_to = "";
        	if (range_fascnrecord_to != null && range_fascnrecord_to.length() > 0)
        		complete_nrecord_to = StringUtil.fillString(range_fascnrecord_to, "0", 7);
        	String query1= addRangeQuery("fasc_nrecord", complete_nrecord_from, complete_nrecord_to, "AND");
    		query +=  query1;
        }
...
//		query +=  addQueryField("fasc_classif", fasc_classif);
		...
		if ( custom_da_numfasc.length() > 0) { //da_numero pieno (ricerca per numero)
			...
			if (custom_classiffasccod.length() > 0 || numUnicaFasc) { //classif pieno o numerazione unica fascicoli
				...
				String queryRangeNumFasc = "";
				for (int i=0; i<numfascLengths.length; i++) {
					String anno = "_ANNOCORRENTE_";
	                if ( fasc_anno.length() > 0 )	//anno_fasc pieno
	                    anno = fasc_anno;

	                String da_numfasc = fillString(custom_da_numfasc, "0", numfascLengths[i]);
	                String a_numfasc = "";
	                if (custom_a_numfasc.length() > 0)
	                	a_numfasc = fillString(custom_a_numfasc, "0", numfascLengths[i]);
	                ...
	                queryRangeNumFasc +=  addQueryField("fasc_numero", query1, "OR");
				}
				if (!queryRangeNumFasc.isEmpty()) {
					queryRangeNumFasc = queryRangeNumFasc.trim();
					if (queryRangeNumFasc.endsWith("OR"))
						queryRangeNumFasc = queryRangeNumFasc.substring(0, queryRangeNumFasc.length()-2);
					
					query +=  query = query +"(" + queryRangeNumFasc + ") AND ";
				}
	        }
            else {
            ...
            if ( query1.length() > 0 ) {
            	...
            	// mbernardini 28/04/2016 : disallineamento query fra dw4 e dw3 in caso di fascicolazione di documenti in fascicoli speciali (task #5211)
            	String fascicoloSpecialeCod = "";
            	if (fascicoloSpecialeInfo != null && fascicoloSpecialeInfo.getClassif() != null
            			&& fascicoloSpecialeInfo.getClassif().getCod() != null && !fascicoloSpecialeInfo.getClassif().getCod().isEmpty())
            		fascicoloSpecialeCod = fascicoloSpecialeInfo.getClassif().getCod();

                if (nonClassificatoValid && !custom_classiffasccod.equals(fascicoloSpecialeCod))
...
//			query += "(([fasc_rifinternirifcodpersona]=\"_CODPERSONA_\") adj ([fasc_rifinternirifdiritto]=RPA)) AND ";
	...
	public String openIndexClassifFascicoloMassiva() throws Exception {
		this.focusElement = "fasc_classif_massiva";
		this.formsAdapter.getIndexForm().addParam("selid", "");
		this.openIndex("fasc_classif", this.fasc_classif, "0", " ", false);
		return null;
	}
	...
	public String switchItfRuoloUfficio() throws Exception {
		this.itftiporuolo = !this.itftiporuolo;
		this.itfnomeuff = "";
		this.itfnomepersona = "";
		return null;
	}
	...
	public String convertClassifCod(String stringa) {
		...
		if (classifFormat != null && !classifFormat.isEmpty()) {
			try {
				...
				Logger.warn("QueryFascicolo.convertClassifCod(): unable to convert " + stringa + " from " + classifFormat + " to numeric format. Maybe already in numeric format...");
			}
		}
	...
	public boolean isItftiporuolo() {
		return itftiporuolo;
	}

	public void setItftiporuolo(boolean itftiporuolo) {
		this.itftiporuolo = itftiporuolo;
	}
	...
	public String getRange_fascnrecord_from() {
		return range_fascnrecord_from;
	}

	public void setRange_fascnrecord_from(String range_fascnrecord_from) {
		this.range_fascnrecord_from = range_fascnrecord_from;
	}

	public String getRange_fascnrecord_to() {
		return range_fascnrecord_to;
	}

	public void setRange_fascnrecord_to(String range_fascnrecord_to) {
		this.range_fascnrecord_to = range_fascnrecord_to;
	}
	...
	public List<FascicoloSpecialeInfo> getFascicoloSpecialeInfoList() {
		return fascicoloSpecialeInfoList;
	}

	public void setFascicoloSpecialeInfoList(List<FascicoloSpecialeInfo> fascicoloSpecialeInfoList) {
		this.fascicoloSpecialeInfoList = fascicoloSpecialeInfoList;
	}
	...
	public String getValueFascicolazioneSpecialeMassiva() {
		return valueFascicolazioneSpecialeMassiva;
	}
	public void setValueFascicolazioneSpecialeMassiva(String valueFascicolazioneSpecialeMassiva) {
		this.valueFascicolazioneSpecialeMassiva = valueFascicolazioneSpecialeMassiva;
	}
	...
	public String resetQuery(){
		...
		ccnomeuff="";
		ccnomepersona="";
		range_fascnrecord_from = "";
		range_fascnrecord_to = "";
	...
	/**
	 * Switch del form di ricerca da generico a specifico del fascicolo speciale per selezione su radio con più fascicoli (fascicolazione massiva)
	 * @return
	 * @throws Exception
	 */
	public String switchToTableFSMassiva() throws Exception {
		try {
			if(!valueFascicolazioneSpecialeMassiva.isEmpty() && !valueFascicolazioneSpecialeMassiva.equals("customSearch")) {
				this.custom_classiffasccod = "";
				this.fasc_classif = "";

				formsAdapter.switchToTableFS(valueFascicolazioneSpecialeMassiva);
				XMLDocumento response = formsAdapter.getDefaultForm().executePOST(getUserBean());
				if (handleErrorResponse(response)) {
					formsAdapter.fillFormsFromResponse(formsAdapter.getLastResponse()); //restore delle form
					return null;
