...
public class DocDocWayDocEditFormsAdapter extends DocEditFormsAdapter {
	...
	protected void fillDefaultFormFromResponse(XMLDocumento response) throws DocumentException {
		...
		//gestione trasformazione in protocollo in arrivo
		if (response.getRootElement().selectSingleNode("/response/doc/@trasformaDocInProtocolloInArrivo")!=null) {
			defaultForm.addParam("trasformaDocInProtocolloInArrivo", true);
			Element doc = (Element) response.getRootElement().selectSingleNode("/response/doc");
			// nel caso di trasformaDocInProtocolloInArrivo, il valore di bozza è forzato, utilizzo attributo draft per gestioni layout
			if(doc != null && doc.attributeValue("draft", "no").toLowerCase().equals("no")) {
				funzionalitaDisponibili.put("trasformaDocInProtocolloInArrivo", true);
			}
		}
	}
