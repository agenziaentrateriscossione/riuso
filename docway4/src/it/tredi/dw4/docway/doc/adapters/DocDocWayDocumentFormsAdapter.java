...
import it.tredi.dw4.docway.beans.DocWayProtocolliMultipli;
...
public class DocDocWayDocumentFormsAdapter extends DocWayDocumentFormsAdapter {
	 ...
	 * Chiamata a firma digitale Uniserv
	 */
	public void firmaUniserv(boolean remotaOtp, String fileId, String fileName, String outputFileType, boolean marcaturaTemporaleRichiesta) {
		defaultForm.addParam("verbo", "showdoc");
		defaultForm.addParam("xverb", "@firmaUniserv");
		defaultForm.addParam("remotaOtp", remotaOtp);
		...
		defaultForm.addParam("marcaturaTemporaleRichiesta", marcaturaTemporaleRichiesta);
	...
	public void firmaRemotaArubaOpenPopup(String outputFileType) {
		defaultForm.addParam("verbo", "showdoc");
		defaultForm.addParam("xverb", "@firmaArubaOpenPopup");
	...
	public void firmaRemotaArubaRichiediOtp(String credenzialeFirmaRemota, String userPassword) {
		defaultForm.addParam("verbo", "showdoc");
		defaultForm.addParam("xverb", "@firmaArubaRichiediOtp");
		
		defaultForm.addParam("credenzialeFirmaRemota", credenzialeFirmaRemota);
		defaultForm.addParam("userPassword", userPassword);
	...
	/**
	 * Chiamata a firma digitale Remota OTP Aruba (ARSS)
	 */
	public void firmaRemotaAruba(String fileId, String fileName, String outputFileType, boolean firmaMultipla, String credenzialeFirmaRemota, String userPassword, String userOtp) {
		defaultForm.addParam("verbo", "showdoc");
		defaultForm.addParam("xverb", "@firmaAruba");
		defaultForm.addParam("firmaMultipla", firmaMultipla);
		...
		defaultForm.addParam("credenzialeFirmaRemota", credenzialeFirmaRemota);
		defaultForm.addParam("userPassword", userPassword);
		defaultForm.addParam("userOtp", userOtp);
	 ...
	 */
	public void trasformaDocInProtocolloInArrivo() {
		this.defaultForm.addParam("verbo", "docEdit");
		this.defaultForm.addParam("xverb", "@trasformaDocInProtocolloInArrivo");
	}

	/**
	 * Richiede i repertori di tipo arrivo selezionabili dall'utente corrente e i file già utilizzati per la protocollazione
	 * multipla s
	 * @param nrecord Nrecord del documento del quale controllare se sono stati utilizzati file
	 */
	public void richiestaProtocollazioneMultipla(String nrecord) {
		this.defaultForm.addParam("verbo", "dwrep");
		this.defaultForm.addParam("xverb", "@protocolliMultipli");
		this.defaultForm.addParam("nrecordUserFiles", nrecord);
	}

	/**
	 * Richiede il salvataggio di copie protocollate della bozzaArrivo per ogni elemento dei protocolliDaCreare, che
	 * contengono informazioni relative al repertorio del protocollo e agli allegati da utilizzare
	 *
	 * @param protocolliDaCreare Lista dei protolli da creare, specificano il repertorio e file da aggiungere
	 * @param bozzaArrivo Documento bozza da protocollare, dal quale prendere le informazioni e i relativi file
	 */
	public void saveProtocolliMultipliDaBozzaArrivo(List<DocWayProtocolliMultipli.ProtocolloArrivo> protocolliDaCreare,
													Doc bozzaArrivo) {
		this.defaultForm.addParam("verbo", "docEdit");
		this.defaultForm.addParam("xverb", "@protocolliMultipli");

		this.defaultForm.addParam("bozzaArrivoNrecord", bozzaArrivo.getNrecord());
		this.defaultForm.addParam("numeroProtocolli", protocolliDaCreare.size());
		for (int i = 0; i < protocolliDaCreare.size(); i++) {
			DocWayProtocolliMultipli.ProtocolloArrivo protocolloArrivo = protocolliDaCreare.get(i);
			this.defaultForm.addParam("repertorio["+i+"]", protocolloArrivo.getSelectedRepertorio());
			StringBuilder serilizedIds = new StringBuilder();
			for (String selectedFile : protocolloArrivo.getSelectedFiles()) {
				serilizedIds.append(selectedFile).append(",");
			}
			serilizedIds = new StringBuilder(serilizedIds.substring(0, serilizedIds.length() - 1));
			this.defaultForm.addParam("files["+i+"]", serilizedIds.toString());
		}
	}

	/**
	 * Ricerca di documenti in base ad una query prestabilita con svuotamento della selezione
	 *
	 * @param query Query da effettuare
	 */
    public void queryDocs(String query) {
    	this.defaultForm.addParam("verbo", "queryPlain");
    	this.defaultForm.addParam("query", query);
    	this.defaultForm.addParam("xverb", "");
		this.defaultForm.addParam("selid", "");
