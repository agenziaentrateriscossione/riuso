...
public class StrutturaEsterna extends XmlEntity {
	...
	private List<ListaDistribuzioneAppartenenza> listaDistribuzione = new ArrayList<>();
	...
	public StrutturaEsterna init(Document domStrutturaEsterna) {
    	...
    	this.listaDistribuzione = XMLUtil.parseSetOfElement(domStrutturaEsterna, "//struttura_esterna/lista_dist_appartenenza", new ListaDistribuzioneAppartenenza());
        ...
        if ( listaDistribuzione.isEmpty() ) listaDistribuzione.add(new ListaDistribuzioneAppartenenza());
        return this;
    }
    ...
    public Map<String, String> asFormAdapterParams(String prefix){
    	...
    	for (int i = 0; i < listaDistribuzione.size(); i++) {
    		ListaDistribuzioneAppartenenza app = (ListaDistribuzioneAppartenenza) listaDistribuzione.get(i);
    		params.putAll(app.asFormAdapterParams(".lista_dist_appartenenza["+String.valueOf(i)+"]"));
		}
	...
	public void clearListaDistribuzione() {
		if (listaDistribuzione != null) {
			listaDistribuzione.clear();
			if (listaDistribuzione.isEmpty())
				this.addListaDistribuzione();
		}
	}
	public String addListaDistribuzione(){
		ListaDistribuzioneAppartenenza appart = (ListaDistribuzioneAppartenenza) FacesContext.getCurrentInstance().getExternalContext().getRequestMap().get("listaDistribuzione");
		int index = listaDistribuzione.indexOf(appart);
		if (listaDistribuzione.size() > index)
			listaDistribuzione.add(index+1, new ListaDistribuzioneAppartenenza());
		else
			listaDistribuzione.add(new ListaDistribuzioneAppartenenza());
		return null;
	}
	public String deleteListaDistribuzione(){
		ListaDistribuzioneAppartenenza appart = (ListaDistribuzioneAppartenenza) FacesContext.getCurrentInstance().getExternalContext().getRequestMap().get("listaDistribuzione");
		this.listaDistribuzione.remove(appart);
		if (listaDistribuzione.isEmpty()) listaDistribuzione.add(new ListaDistribuzioneAppartenenza());
		return null;
	}
	public String moveUpListaDistribuzione(){
		ListaDistribuzioneAppartenenza appart = (ListaDistribuzioneAppartenenza) FacesContext.getCurrentInstance().getExternalContext().getRequestMap().get("listaDistribuzione");
		int index = listaDistribuzione.indexOf(appart);
		if (index > 0 ) {
			listaDistribuzione.remove(index);
			this.listaDistribuzione.add(index-1, appart);
		}
		return null;
	}

	public String moveDownListaDistribuzione(){
		ListaDistribuzioneAppartenenza appart = (ListaDistribuzioneAppartenenza) FacesContext.getCurrentInstance().getExternalContext().getRequestMap().get("listaDistribuzione");
		int index = listaDistribuzione.indexOf(appart);
		if (index < listaDistribuzione.size()-1 ) {
			listaDistribuzione.remove(index);
			this.listaDistribuzione.add(index+1, appart);
		}
		return null;
	...
	public List<ListaDistribuzioneAppartenenza> getListaDistribuzione() {
		return listaDistribuzione;
	}

	public void setListaDistribuzione(List<ListaDistribuzioneAppartenenza> listaDistribuzione) {
