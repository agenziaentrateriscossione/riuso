...
public class DocEditPersonaEsterna extends AclDocEdit {
	...
	public String lookupListaDistribuzioneAppartenenza() throws Exception {
		...
		int num = (persona_esterna.getListaDistribuzione().contains(listaDistribuzione)) ? persona_esterna.getListaDistribuzione().indexOf(listaDistribuzione): 0;
		...
		String campi 		= ".listaDistribuzione["+num+"].@cod=xml,/lista_dist/@nrecord ; .listaDistribuzione["+num+"].@nome=xml,/lista_dist/nome ; .listaDistribuzione["+num+"].@physDoc=xml,/lista_dist/@physDoc"; //campi
		String newRecord 	= ""; //newRecord 
		
		String value = (persona_esterna.getListaDistribuzione() != null && persona_esterna.getListaDistribuzione().get(num) != null) ? persona_esterna.getListaDistribuzione().get(num).getNome() : "";
		callLookup(persona_esterna, aliasName, aliasName1, titolo, ord, campi, xq, db, newRecord, value);
		return null;
	}
