...
public class DocEditFormsAdapter extends FormsAdapter {
	...
	protected void fillDefaultFormFromResponse(XMLDocumento response) throws DocumentException {
		...
		/* per le deleghe titles */
		Element titlesBacktrack = (Element) root.selectSingleNode("/response/titlesBackTracks");
		if (titlesBacktrack != null) {
			defaultForm.addParam("pos", titlesBacktrack.attributeValue("pos", "0"));
			defaultForm.addParam("ripropz", titlesBacktrack.attributeValue("ripropz", ""));
		}
	}
...
//TODO - completare
	...
	protected void unlockDocument(String xverb) {
	...
	public void protocollaDoc(String bozzaDaProt, String codRepertorio, String descrRepertorio) {
		...
		this.defaultForm.addParam("protocollaFromDocEdit", "true");
	}
	...
	public void confirmDelega(Delega delega, String matricolaRiferimento) {
		this.defaultForm.addParam("verbo", "deleghe");
		this.defaultForm.addParam("xverb", "@saveDelega");
		this.defaultForm.addParam("delega_permanente", Boolean.toString(delega.isPermanente()));
		this.defaultForm.addParam("delega_uneditable", Boolean.toString(delega.isUneditable()));
		this.defaultForm.addParam("delega_attiva", Boolean.toString(delega.isAttiva()));
		this.defaultForm.addParam("delega_sostituto", Boolean.toString(delega.isSostituto()));
		this.defaultForm.addParam("delega_startDate", delega.inizioToLocalDate() != null ? delega.inizioToLocalDate().toString() : "");
		this.defaultForm.addParam("delega_endDate", delega.fineToLocalDate() != null ? delega.fineToLocalDate().toString() : "");
		this.defaultForm.addParam("delega_cod_riferimento", matricolaRiferimento);
		// ciclo e concateno tutti gli nrecord delle persone delegate (per inviarle al service)
		String delegati = "";
		for(Rif rif : delega.getRifDelegati()) {
			if(rif.getCod_persona() != null && !rif.getCod_persona().isEmpty())
				delegati+="|" + rif.getCod_persona() + "$" + rif.getNome_persona() + "$" + rif.getCod_uff() + "$" + rif.getNome_uff();
		}
		this.defaultForm.addParam("delegati", delegati);
	}
	
	public void editDelegante(Delega delega, String matricolaRiferimento) {
		this.defaultForm.addParam("verbo", "deleghe");
		this.defaultForm.addParam("xverb", "@saveDelegante");
		this.defaultForm.addParam("delega_permanente", Boolean.toString(delega.isPermanente()));
		this.defaultForm.addParam("delega_uneditable", Boolean.toString(delega.isUneditable()));
		this.defaultForm.addParam("delega_attiva", Boolean.toString(delega.isAttiva()));
		this.defaultForm.addParam("delega_sostituto", Boolean.toString(delega.isSostituto()));
		...
		this.defaultForm.addParam("delega_endDate", delega.fineToLocalDate() != null ? delega.fineToLocalDate().toString() : "");
		this.defaultForm.addParam("delega_cod_riferimento", matricolaRiferimento);
		// estraggo i dati del delegante, inserito nella lista dei rif per comodità (per inviarle al service)
		Rif rifDelegante = delega.getRifDelegati().get(0);
		String delegante = rifDelegante.getCod_persona() + "$" + rifDelegante.getNome_persona() + "$" + rifDelegante.getCod_uff() + "$" + rifDelegante.getNome_uff();
		this.defaultForm.addParam("delegante", delegante);
	}
	
	public void removeDelega(Delega delega, String matricolaRiferimento) {
		this.defaultForm.addParam("verbo", "deleghe");
		this.defaultForm.addParam("xverb", "@removeDelega");
		this.defaultForm.addParam("matricolaToRemove", delega.getCodPersona());
		this.defaultForm.addParam("delega_cod_riferimento", matricolaRiferimento);
	}
	
	public void removeDelegante(Delega delega, String matricolaRiferimento) {
		this.defaultForm.addParam("verbo", "deleghe");
		this.defaultForm.addParam("xverb", "@removeDelegante");
		this.defaultForm.addParam("matricolaDelegante", delega.getCodPersona());
		this.defaultForm.addParam("delega_cod_riferimento", matricolaRiferimento);
	}

	public void reloadDelegheOptions(String matricolaPersonaInt) {
		defaultForm.addParam("verbo", "deleghe");
    	defaultForm.addParam("xverb", "@openDeleghePersona");
		if (matricolaPersonaInt != null && !matricolaPersonaInt.isEmpty())
			defaultForm.addParam("matricolaPersonaInt", matricolaPersonaInt);
	}

	public void toggleSostituto(Delega delega, String matricolaPersonaInt) {
		defaultForm.addParam("verbo", "deleghe");
		defaultForm.addParam("xverb", "@toggleSostituto");
		defaultForm.addParam("newFlag", Boolean.toString(delega.isSostituto()));
		defaultForm.addParam("matricolaToToggle", delega.getCodPersona());
		if (matricolaPersonaInt != null && !matricolaPersonaInt.isEmpty())
			defaultForm.addParam("matricolaPersonaInt", matricolaPersonaInt);
	}

	public void backToPaginaTitoli() {
	...
	/**
	 * Aggiunge informazioni riguardanti il cambio dei rif_esterni di un protocollo.
	 * @param historyLabel Label da aggiungere alla storia del documento
	 */
	public void addRifEsterniChangedInfos(String historyLabel) {
		defaultForm.addParam("rifEsterniProtocolloChanged", "true");
		if (historyLabel != null)
			defaultForm.addParam("rifEsterniChangedHistoryLabel", historyLabel);
	}
